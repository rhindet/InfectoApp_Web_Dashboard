...
import { 
  Bold, Italic, Underline, List, ListOrdered, Save, X, Table, PlusCircle 
} from 'lucide-react';
...
const ArticleEditor: React.FC<ArticleEditorProps> = ({ article, onSave, onCancel }) => {
  ...
  // Dropdown states (ocultos, pero funcionales)
  ...
  const insertTableWithExec = () => {
    ...
    const html = `<table style="width:100%;border-collapse:collapse;margin:8px 0;">${header}${body}</table><br/>`;
    ...
    <div className="bg-white rounded-lg shadow-sm border">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-2 mb-4 p-2 bg-gray-50 rounded-lg">
          ...
          <button type="button" onClick={() => formatText('insertUnorderedList')} className="p-2 hover:bg-gray-200 rounded" title="Lista con viñetas">
            <List className="w-4 h-4" />
          </button>
          <button type="button" onClick={() => formatText('insertOrderedList')} className="p-2 hover:bg-gray-200 rounded" title="Lista numerada">
