...
type ModalMoveDialogProps = {
  isOpen: boolean;
  itemName: string;              // “Terraria”
  currentLocationLabel?: string; // “My Drive”
  onClose: () => void;
  onMove: (targetFolderId: NodeId) => void;
  loadChildren: LoadChildren;    // <-- la provee Dashboard dinámicamente
  rootId?: NodeId | null;        // null = raíz
};

export const ModalMoveDialog: React.FC<ModalMoveDialogProps> = ({
  isOpen,
  itemName,
  currentLocationLabel = "Articulos ruta",
  onClose,
  onMove,
  loadChildren,
  rootId = null,
  ...
  // Helper: parsear nivel e id
  const parseLevelAndId = (fullId: string) => {
    const [prefix, rawId] = fullId.split(":");
    const m = /^L(\d+)$/.exec(prefix ?? "");
    const level = m ? Number(m[1]) : null;
    return { level, id: rawId ?? "", fullId };
  };

  // Log de hoja (sin hijos)
  useEffect(() => {
    if (!isOpen) return;
    const current = path.at(-1);
    const currentId = (current?.id ?? rootId) as string | null;

    if (!loading && currentId && nodes.length === 0) {
      const { level, id, fullId } = parseLevelAndId(currentId);
      const name = current?.name ?? "(root)";
      console.log("Leaf reached (no children):", { fullId, level, id, name });
    }
  }, [nodes, loading, isOpen, path, rootId]);

  if (!isOpen) return null;

  // navegación “entrar a carpeta”
  ...
  return createPortal(
    ...
    >
      <div
        className="w-[720px] max-w-[95vw] max-h-[85vh] bg-white rounded-xl shadow-xl overflow-hidden"
